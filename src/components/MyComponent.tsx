...
const MyComponent: React.FC<MyComponentProps> = ({ testtype }) => {
  ...
  const QtDiv = () => {
    ...
    return (
      <div>
        <p>Positive Wells</p>
        <input type="numeric" min={0} max={51} value={inputQt} defaultValue={0}
               onChange={handleChangeQt}/>
        {testtype === 'quanti' && (mpnVal !== undefined && <><h1>MPN : {mpnVal[0]}</h1>
  ...
  const Qt2kDiv = () => {
    ...
    const mpnVal = getQt2KMpn(inputQt2kL, inputQt2kS) !== undefined ? getQt2KMpn(inputQt2kL, inputQt2kS): [];
    ...
    return (
      <div>
        ...
        <div className="flex flex-row space-x-1.5">
          <div>
            <p>Large</p>
            <input type="numeric" min={0} max={49} value={inputQt2kL} defaultValue={0}
                   onChange={handleChangeQt2kL}/>
          </div>
          <div>
            <p>Small</p>
            <input type="numeric" min={0} max={49} value={inputQt2kS} defaultValue={0}
                   onChange={handleChangeQt2kS}/>
          </div>
        </div>
        {testtype === 'quanti2k' && (<>
          <div><h1>MPN : {mpnVal !== undefined && mpnVal[0]}</h1></div>
          <div>95% Confidence</div>
          <div className="flex row space-x-1.5">
            <div>Low: {mpnVal && mpnVal[1]}</div>
            <div>High: {mpnVal && mpnVal[2]}</div>
  ...
  const QtLDiv = () => {
    ...
    return (<div>
        ...
        <div className="flex flex-row space-x-1.5">
          <div><p>Large</p>
            <input type="numeric" min={0} max={6} value={inputLL} defaultValue={0}
                   onChange={handleChangeLL}/></div>
          <div>
            <p>Small</p>
            <input type="numeric" min={0} max={90} value={inputLS} defaultValue={0}
                   onChange={handleChangeLS}/>
          </div>

        </div>
        {testtype === 'legiolert' && <h1>MPN : {getQtLegio(inputLL, inputLS)}</h1>}
      </div>
    );

  }
  return testtype === 'quanti' ? (<div>{QtDiv()}</div>) :
    testtype === 'quanti2k' ? (<div>{Qt2kDiv()}</div>) : (<div>{QtLDiv()}</div>);
};
